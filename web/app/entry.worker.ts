...
const handlePush = (event: PushEvent) => {
  ...
  const title = data.title ? data.title : "M-GO";

  const options = {
    body: data.body ? data.body : "Notification Body Text",
    icon: data.icon ? data.icon : "/icons/manifest-icon-192.maskable.png",
    badge: data.badge ? data.badge : "/icons/manifest-icon-512.maskable.png",
