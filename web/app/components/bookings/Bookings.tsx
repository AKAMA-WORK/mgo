...
import SeatsLegend from "../departures/SeatsLegend";
import { Link } from "@remix-run/react";
import { Booking } from "~/types/booking/Booking";
import { formatAmount } from "~/utils/amount-util";
import moment from "moment";

interface BookingPreviewIconProps {
    icon: ReactNode
    name: string
    value: string
}

const BookingPreviewIcon = ({ icon, name, value }: BookingPreviewIconProps) => {

    return (
        <div className="app-app-departure-preview-icon">
            <div>{icon}</div>
            <div className="app-app-departure-preview-icon-name">
                <div className="app-app-departure-preview-icon-name-name">
                    {name}
                </div>
                <div className="app-app-departure-preview-icon-name-value">
                    {value}
                </div>
            </div>
            {/* <div>{value}</div> */}
...
const BookingPreview = ({ booking, seatsSelection, showConfirmButton }: BookingPreviewProps) => {
    ...
    const price = (
        <div className="app-departure-preview-body-price">
            <div className="app-departure-preview-body-price-item app-departure-preview-body-price-last">
                <div className="app-departure-preview-body-price-item-title">Montant</div>
                <div className="app-departure-preview-body-price-item-content">{formatAmount(booking.amount)} Ar</div>
            </div>

            <div className="app-departure-preview-body-price-action">
                <Form method="post" action="/">
                    <Button type="primary" htmlType="submit">Imprimer ticket</Button>
                </Form>

                <Form method="post" action="/">
                    <Button type="default" htmlType="submit">Annuler réservation</Button>
                </Form>
            </div>
        ...
        <div className="app-departure-preview-body-description-extra">

            {
                booking.lines.map((line) => (
                    <>
                        <div>
                            <h1>{line.departure.organization.name}</h1>
                        </div>
                        <div>
                            <span className="app-departure-preview-body-description-extra-title">{line.departure.from.name} - {line.departure.to.name} / {moment.utc(line.departure.date).format('dddd DD MMMM YYYY')} à {line.departure.time}</span>
                        </div>
                    </>
    ...
    return (
        <>
            <Card
                ...
                title={
                    <div className="app-departure-preview-title">
                        <div className="app-departure-preview-title-company">{booking.lines[0].departure.from.name}</div> -
                        <div className="app-departure-preview-title-company">{booking.lines[0].departure.to.name}</div>
                        {!seatsSelection && <div className="app-departure-preview-title-company-voir-plus">
                            <Link to={`/booking/${booking.bookingId}`}>
                                <Button  type="text">Voir détail</Button>
                            </Link >
                        </div>
                        }
                    </div>
                    ...
                    <div className="app-departure-preview-body-description">
                        <div className="app-departure-preview-body-description-vehicule">
                            <BookingPreviewIcon
                                ...
                                value={booking.paymentDate ? moment.utc(booking.paymentDate).format('DD/MM/YYYY'):''}
                            />
